...
export function CollectionEpisodeTab() {
    const [searchKey, setSearchKey] = useState<string>("");

    const [pageIndex, setPageIndex] = useState(0);
    const [pageSize, setPageSize] = useState(9);

    const {data: page} = useSWR<PageType<MovieType>>(`/api/v1/collections?type=TV&q=${searchKey}&pageIndex=${pageIndex}&pageSize=${pageSize}`, get);
                        ...
                        <Link href={`/movies/${movie.id}`} style={{textDecoration: "none"}}>
                            ...
                            >
                                <Stack spacing={1}>
                                    ...
                                    <Grid item xs>
                                        <Typography gutterBottom variant="subtitle1" component="div" sx={{m: 1}}>
                                            <b>{movie.season_name} - {movie.episode_name}</b>
                                        </Typography>
                                    </Grid>
