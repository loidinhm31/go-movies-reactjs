...
import VideoPlayer, { VideoJsOption } from "@/components/Player/VideoPlayer";
        ...
        <>
          ...
          <Box sx={{ p: 1, m: 1 }}>
            <Typography>
              {episode && (
                <small>
                  <em>{format(new Date(episode?.air_date!), "MMMM do, yyyy")} | </em>
                </small>
              )}
              <small>
                <em>{episode?.runtime} minutes | </em>
              </small>
              <small>
          ...
          <Stack direction="row" justifyContent="space-between">
            ...
            <BuyCollection movie={movie} episode={episode} />
          </Stack>
          <Divider />
          ...
          <Stack component="span" sx={{ display: "flex", justifyContent: "center", p: 1, m: 1 }}>
            <Grid container spacing={3}>
                ...
                <Stack>
                  {movie.image_url !== "" && (
                    <Box sx={{ display: "flex", justifyContent: "center", width: 1, height: 1 }}>
                      <CardMedia sx={{ borderRadius: "16px" }} component="img" src={movie.image_url} alt="poster" />
                    </Box>
                  )}
                </Stack>
              </Grid>
              ...
              <Grid item xs={12} md={10}>
                <Stack spacing={2}>
                  <Box sx={{ m: 1 }}>
                    <Views wasMutateView={mutateView} setWasMuateView={setMutateView} movieId={movie.id!} />
                  </Box>
                  <Typography sx={{ m: 2 }} variant="body1">
                    {movie.description}
                  </Typography>

                  {!episodeId && movie.type_code === "TV" && <SeasonInformation movieId={movie.id!} />}
                </Stack>
              </Grid>
            </Grid>
          </Stack>
